...
const PnlCard = () => {
  ...
  const { data, isLoading, isError } = usePnL({ month: selectedMonth });
  ...
  const selected: PnL = useMemo(() => {
    ...
    return {
      ...base,
      ...data,
      salary: { ...base.salary, ...data.salary },
      sales: { ...base.sales, ...data.sales },
      fixedCost: data.fixedCost ?? base.fixedCost,
      variableCost: data.variableCost ?? base.variableCost,
    };
  }, [data, selectedMonth]);
  const isProfit = selected.profit >= 0;
