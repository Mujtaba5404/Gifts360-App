...
type EditPORouteProp = RouteProp<RootStackParamList, 'EditPurchaseOrder'>;
...
const EditPurchaseOrder = () => {
  ...
  const route = useRoute<EditPORouteProp>();
  ...
  // Jab tak record load ho ke aa nahi jata, form ko pre-fill nahi karna
  const [isFormReady, setIsFormReady] = useState(false);

  useEffect(() => {
    if (!purchaseOrderData) return;

    const po: any = purchaseOrderData.data ?? purchaseOrderData;

    setVendorId(po.vendor?._id ?? po.vendor ?? '');

    const mappedRows: OrderItemRow[] = (po.items ?? []).map((it: any) => ({
      item: it.item,
    ...
    setTaxAmount(String(po.taxAmount ?? '0'));
    setServiceOrDeliveryFee(String(po.serviceOrDeliveryFee ?? '0'));
    setOrderDate(po.orderDate ? new Date(po.orderDate) : new Date());
    setInvoiceSubmissionDate(
      po.invoiceSubmissionDate ? new Date(po.invoiceSubmissionDate) : undefined,
    );
    setPaymentStatus(po.paymentStatus ?? 'pending');
    setPaymentDate(po.paymentDate ? new Date(po.paymentDate) : undefined);
    setIsReceived(!!po.isReceived);
    setReceivedOn(po.receivedOn ? new Date(po.receivedOn) : undefined);
    setNotes(po.notes ?? '');
  ...
  // Profile edit page ki tarhan: jab tak record fetch ho raha hai, loader dikhao
  if (isLoadingOrder || !isFormReady) {
    return (
      <View style={styles.container}>
        <TopHeader text="Edit Purchase Order" isBack />
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <CustomButton
          text={isPending ? 'Updating...' : 'Update Purchase Order'}
          onPress={onSave}
          disabled={isPending}
